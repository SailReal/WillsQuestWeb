...
class FunctionalSpec
    ...
    "return ok on `onHelp`" in new Context {
      new WithApplication(application) {
        ...
        val Some(result) =
          route(app,
                addCSRFToken(
                  FakeRequest(routes.RestController.onHelp())
    ...
    "return ok on `onStartGame`" in new Context {
      new WithApplication(application) {
        val Some(result) =
          route(app,
                addCSRFToken(
                  FakeRequest(routes.RestController.onStartGame())
                    .withAuthenticator[DefaultEnv](identity.loginInfo)))
    ...
    "return ok on `onAnswerChosen`" in new Context {
      new WithApplication(application) {
        val Some(result) =
          route(app,
                addCSRFToken(
                  FakeRequest(routes.RestController.onAnswerChosen(2))
                    .withAuthenticator[DefaultEnv](identity.loginInfo)))
    ...
    "return ok on `onAddPlayer`" in new Context {
      new WithApplication(application) {
        val Some(result) =
          route(app,
                addCSRFToken(
                  FakeRequest(routes.RestController.onAddPlayer("testPlayer"))
                    .withAuthenticator[DefaultEnv](identity.loginInfo)))
    ...
    "return ok on `onRemovePlayer`" in new Context {
      new WithApplication(application) {
        val Some(result) = route(
          app,
          addCSRFToken(
            FakeRequest(routes.RestController.onRemovePlayer("testPlayer"))
              .withAuthenticator[DefaultEnv](identity.loginInfo)))
    ...
    "return 2 on `getMaxPlayerCount`" in new Context {
      new WithApplication(application) {
        val Some(result) =
          route(app,
                addCSRFToken(
                  FakeRequest(routes.RestController.getMaxPlayerCount())
                    .withAuthenticator[DefaultEnv](identity.loginInfo)))
    ...
    "return <unknown> on `getUsername`" in new Context {
      new WithApplication(application) {
        val Some(result) =
          route(app,
                addCSRFToken(
                  FakeRequest(routes.RestController.getUsername())
                    .withAuthenticator[DefaultEnv](identity.loginInfo)))
    ...
    "return ok on `onReset`" in new Context {
      new WithApplication(application) {
        val Some(result) =
          route(app,
                addCSRFToken(
                  FakeRequest(routes.RestController.onReset())
                    .withAuthenticator[DefaultEnv](identity.loginInfo)))
  ...
  "The `notfound` action" should {
    "should show not found" in new Context {
      new WithApplication(application) {
        val Some(result) = route(
          app,
          FakeRequest(routes.NotFoundController.index("foo"))
    ...
    "redirect to login page if user is unauthorized" in new Context {
      new WithApplication(application) {
        val Some(redirectResult) = route(
          app,
          FakeRequest(routes.HomeController.index())
            .withAuthenticator[DefaultEnv](LoginInfo("invalid", "invalid")))
    ...
    "redirect if user was logged out" in new Context {
      new WithApplication(application) {
        val Some(result) =
          route(app,
                addCSRFToken(
                  FakeRequest(routes.HomeController.signOut())
                    .withAuthenticator[DefaultEnv](identity.loginInfo)))
    ...
    "return ok on signup page" in new Context {
      new WithApplication(application) {
        val Some(result) = route(
          app,
          addCSRFToken(
            FakeRequest(routes.SignUpController.view())
              .withAuthenticator[DefaultEnv](LoginInfo("invalid", "invalid"))))
    ...
    "return redirect on signup page" in new Context {
      new WithApplication(application) {
        val Some(result) = route(
          app,
          addCSRFToken(
            FakeRequest(routes.SignUpController.submit())
    ...
    "return redirect on signin page" in new Context {
      new WithApplication(application) {
        val Some(_) = route(
          app,
          addCSRFToken(
            FakeRequest(routes.SignInController.view())
              .withFormUrlEncodedBody("username" -> "foo", "password" -> "bar")
              .withAuthenticator[DefaultEnv](LoginInfo("invalid", "invalid")))
        )

        val Some(resultSignin) = route(
          app,
          addCSRFToken(
            FakeRequest(routes.SignInController.submit())
              .withFormUrlEncodedBody("username" -> "foo", "password" -> "bar")
  ...
  "The `Websocket` action" should {
    "not fail" in new Context {
      new WithApplication(application) {
        route(app,
              addCSRFToken(
                FakeRequest(routes.WebsocketController.createSocket())
                  .withAuthenticator[DefaultEnv](identity.loginInfo)))
      }
    }
  }
  ...
  trait Context extends Scope {
    ...
    lazy val mockedServerController: Controller =
      MockedServerController.create()
